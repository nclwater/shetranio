...
#each figure shows all the points at a particular time. There is a seperate figure for each time
#++++++++++++++++++++++++++++++++++++++++
#THINGS TO CHANGE
...
#outfile location
outfilefolder = '6-ouptut-Soil-moisture-profile'
    ...
    while time<ntimes:
        ...
        while points<Npoints:
            if elevation[points]!=-1:
                plotlabel[points]='Col='+str(int(col[points]))+' Row='+str(int(row[points]))+' Elev= %7.2f m'%elevation[points]
                ax.plot(data[0:userspecNlayers-1,time,points],depth[0:userspecNlayers-1],label=plotlabel[points])
            points+=1
        axes = plt.gca()
        ...
        plt.savefig(outfilefolder+'/'+'profile'+str(time)+'.png')
        plt.close()
        time+=1
